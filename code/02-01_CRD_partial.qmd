...
# c) Wrangling 
# rep, nrate, and krat,e should be considered as categorical(factor) and not numerical(double)
```{r crd_dfw}
crd_dfw <- crd_df %>%
...
ggplot(crd_dfw, aes( x= nrate_kgha,
  ...
  geom_boxplot() +
  geom_jitter() +
  facet_grid(.~krate_kgha)  # take this column and for each level of this column make a separate plot 
```
...
ggplot(crd_resid, aes(x=.fitted, y=.studresid))+
  ...
  theme_bw()
```
Looks great! (no clear pattern in one direction) Next.  
...
crd_means_all
```
emmean -> expected marginal means
CL -> confidence levels

```{r interaction means n inside k}
#to find best N rate inside K rate
...
# to find the best K rate inside N rate
...
#cld -> compact laddr display
crd_cld_all <- cld(crd_means_all,
                   reverse = T,
                   Letters = letters,
                   adjust = "none" # it can also be "tukey"
                   )
